...
class SibylClientTcpBinProtocol(Protocol):
    ...
    def dataReceived(self, line):
        ...
        line=buffer+line
        if len(line)<6:
            buffer=line #we should wait
        else: # on cannais à ce stade la longuer de notre message
            length_msg=fct.unpack('LH',line)[1]-6
            if length_msg<=len(line[6:]): # message complet
                msg = fct.unpack(str(length_mess) +'s', line)[0]
