...
class SibylClientUdpTextProtocol(DatagramProtocol):
    ...
    def sendRequest(self, line):
        ...
        """
        temps=int(time.time())
        msg=str(temps)+': '+line +chr(13) +chr(10)
        print(msg)
        self.transport.write(msg.encode('utf-8'),(self.serverAddress,self.serverPort))
    ...
    def datagramReceived(self, datagram, host_port):
        ...
        [time,reponse]=data.split(' ',1)
        reponse=reponse[:-2]
        print(reponse)
        self.clientProxy.responseReceived(reponse)
