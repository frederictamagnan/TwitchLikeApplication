...
class SibylServerUdpBinProtocol(DatagramProtocol):
    ...
    def datagramReceived(self, datagram, host_port):
        ...
        """
        print datagram #datagram:time+length+message
        time_sent,length_datagram =unpack('!LH',datagram)
        length_message=length_datagram-6 #longeur de la question
        time_sent,length_datagram,qst=unpack('!LH'+str(length_message)+'s',datagram)
        print(time_sent,length_message,qst)
        rps=self.sibylServerProxy.generateResponse(qst)
        print(rps)
        buff=bytearray(len(rps)+6)
        buff=build(rps,time_sent)
        reactor.callLater(int(log(length_message))+1,self.transport.write,buff,host_port)
